...
.evozeus/wrapper.json exists and wrapper version is known
```
...
- 目标 Skill 诊断时读取 `.evozeus/wrapper.json`。
...
5. 新增 `.evozeus/wrapper.json` 记录 wrapper version、managed files、canonical repo、install links。
