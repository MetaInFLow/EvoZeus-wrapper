...
python3 scripts/evozeus_wrapper.py harness migrate-layout --target /absolute/path/to/my-skill --latest-version v0.9.0 --dry-run --json
python3 scripts/evozeus_wrapper.py harness migrate-layout --target /absolute/path/to/my-skill --latest-version v0.9.0 --json
```
...
- Structure：目标 repo 必须包含 `.evozeus-wrapper/` canonical harness。说明入口由 manifest 的 `instruction_surface` 或根 `SKILL.md` / `AGENTS.md` 决定；`integration.mode=native_host_hook` 时必须有宿主 hook 证据；`onboarding` 必须完整记录安装、调用、初始化和子 Skill hook 契约。
...
- Harness：`harness upgrade-check` 默认从 GitHub latest release 获取权威最新版本；查询失败时返回 `latest_unknown`，不得把当前版本当成最新版本。旧 layout 先运行 `harness migrate-layout --dry-run`，无冲突并获批准后再迁移。
