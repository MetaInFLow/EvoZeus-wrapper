...
- 原 `SKILL.md` 会在 frontmatter 后加入 wrapper-owned 状态检查区，再追加自进化方法和 `EvoZeus-wrapper` 路由说明；这些内容不改写业务规则。
...
python3 scripts/evozeus_wrapper.py harness upgrade-check --target /absolute/path/to/my-skill --latest-version v0.3.0 --json
python3 scripts/evozeus_wrapper.py harness upgrade --target /absolute/path/to/my-skill --latest-version v0.3.0 --dry-run --json
```
...
同时，脚本会写入 `.evozeus/wrapper.json`，并在目标 Skill 根目录 `SKILL.md` 的 frontmatter 后加入 `EvoZeus-wrapper 状态检查`，再追加“自进化方法”和 `EvoZeus-wrapper` 章节。新增内容只说明状态检查、wrapper harness 的路由、版本和迁移规则，不改写原 Skill 业务规则。
...
- Structure：目标 repo 必须包含 wrapper 文件，且根目录 `SKILL.md` frontmatter 后必须先出现 `EvoZeus-wrapper 状态检查`，再描述自进化方法和 canonical repo pointer。
...
- Harness：目标 repo 的 `.evozeus/wrapper.json` 记录 wrapper harness version；Skill release 与 wrapper version 必须分开检查。wrapper 版本升级时，先生成迁移方案，记录到 `docs/wrapper-migrations/`；`SKILL.md` 先确保 frontmatter 后存在状态检查，其他 wrapper 内容只追加 `EvoZeus-wrapper` 区域或 migration note，不重写目标 Skill 业务段落。
