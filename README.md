...
- 增加 `.evozeus/wrapper.json`，记录 wrapper version、managed files、canonical repo、install links 和 runtime integration mode。
...
同时，脚本会写入 `.evozeus/wrapper.json`，记录 surface diagnosis 选中的 `instruction_surface` 和 `integration.mode`，并在该 surface 加入 `EvoZeus-wrapper 状态检查`，再追加“自进化方法”和 `EvoZeus-wrapper` 章节。新增内容只说明状态检查、wrapper harness 的路由、版本和迁移规则，不改写原业务规则。
