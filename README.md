...
python3 scripts/evozeus_wrapper.py harness upgrade-check --target /absolute/path/to/my-skill --latest-version v0.8.0 --json
python3 scripts/evozeus_wrapper.py harness migrate-layout --target /absolute/path/to/my-skill --latest-version v0.8.0 --dry-run --json
python3 scripts/evozeus_wrapper.py harness migrate-layout --target /absolute/path/to/my-skill --latest-version v0.8.0 --json
```
...
同时，脚本会把 dashboard、changelog、design docs、policy、preflight 和 hook adapter 统一写入 `.evozeus-wrapper/`，并在 `.evozeus-wrapper/wrapper.json` 记录 `instruction_surface` 和 `integration.mode`。目标说明面只增加状态检查、自进化方法和 wrapper 路由，不改写原业务规则。
...
如果选择 `private`，把 `--public` 改成 `--private`。注意：GitHub Pages 对 private repo 的可用性取决于 GitHub plan，而且 Pages 站点本身可能仍是互联网可访问的发布面；不要把敏感内容放进 `.evozeus-wrapper/docs/`。
...
- Structure：目标 repo 必须包含 `.evozeus-wrapper/` canonical harness。说明入口由 manifest 的 `instruction_surface` 或根 `SKILL.md` / `AGENTS.md` 决定；`integration.mode=native_host_hook` 时必须有宿主 hook 证据。
...
- Harness：wrapper 版本升级先运行 `harness migrate-layout --dry-run`。无冲突并获批准后执行迁移，把旧散落文件移动到 `.evozeus-wrapper/`，重写引用，写入 layout v2 manifest 和 migration record，再运行 structure/runtime 验证。
- Start hook：`.codex/hooks.json` 调用 `.evozeus-wrapper/hooks/evozeus_wrapper_start_check.py`；前者是宿主薄接点，后者是 wrapper 自有实现。
