...
- 原 `SKILL.md` 会追加自进化方法和 `EvoZeus-wrapper` 路由说明，但不改写业务规则。
...
python3 scripts/evozeus_wrapper.py harness upgrade-check --target /absolute/path/to/my-skill --latest-version v0.2.0 --json
python3 scripts/evozeus_wrapper.py harness upgrade --target /absolute/path/to/my-skill --latest-version v0.2.0 --dry-run --json
```
...
同时，脚本会写入 `.evozeus/wrapper.json`，并在目标 Skill 根目录 `SKILL.md` 中追加“自进化方法”和 `EvoZeus-wrapper` 章节。追加内容只说明 wrapper harness 的路由、版本和迁移规则，不改写原 Skill 业务规则。
...
- Harness：目标 repo 的 `.evozeus/wrapper.json` 记录 wrapper harness version；Skill release 与 wrapper version 必须分开检查。wrapper 版本升级时，先生成 append-only 迁移方案，记录到 `docs/wrapper-migrations/`；`SKILL.md` 只追加 `EvoZeus-wrapper` 区域或 migration note，不重写目标 Skill 业务段落。
