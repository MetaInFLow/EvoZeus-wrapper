...
The hook should continue only when `decision.allow` is true. A `warn` result may continue in advisory mode; a `block` result must stop before the target Skill main chain.
